...
class Dashboard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return CustomScrollView(slivers: [
      SliverAppBar(title: const Text('ExamPrep', style: TextStyle(fontWeight: FontWeight.bold)), actions: [IconButton(onPressed: store.toggleTheme, icon: Icon(store.darkMode ? Icons.light_mode : Icons.dark_mode))], floating: true),
  ...
  Widget _progressCard(BuildContext context) {
    final colors = Theme.of(context).colorScheme;
    final progress = store.dailyGoal == 0 ? 0.0 : (store.solvedToday / store.dailyGoal).clamp(0.0, 1.0).toDouble();
    return Card(color: colors.primary, child: Padding(padding: const EdgeInsets.all(20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text("Today's progress", style: TextStyle(color: colors.onPrimary.withAlpha(205))),
      const SizedBox(height: 8),
      Row(crossAxisAlignment: CrossAxisAlignment.end, children: [Text('${store.solvedToday}', style: TextStyle(fontSize: 36, fontWeight: FontWeight.bold, color: colors.onPrimary)), Text(' / ${store.dailyGoal} questions', style: TextStyle(color: colors.onPrimary.withAlpha(205)))]),
      const SizedBox(height: 14),
      ClipRRect(borderRadius: BorderRadius.circular(10), child: LinearProgressIndicator(value: progress, minHeight: 8, backgroundColor: colors.onPrimary.withAlpha(50), color: colors.onPrimary)),
      const SizedBox(height: 16),
      Row(children: [Text('${(store.accuracy * 100).round()}% accuracy', style: TextStyle(color: colors.onPrimary)), const Spacer(), Text('${store.correct} correct • ${store.wrong} wrong', style: TextStyle(color: colors.onPrimary.withAlpha(205)))])
    ])));
  }
...
class _QuestionBankState extends State<QuestionBank> {
  ...
  Widget build(BuildContext context) {
    ...
    final chips = ['All', ...subjects].map((subject) => Padding(padding: const EdgeInsets.only(right: 8), child: ChoiceChip(label: Text(subject), selected: filter == subject, onSelected: (_) => setState(() => filter = subject)))).toList();
    return Column(children: [
      ...
      SizedBox(height: 45, child: ListView(scrollDirection: Axis.horizontal, padding: const EdgeInsets.symmetric(horizontal: 16), children: chips)),
...
class _QuizPageState extends State<QuizPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final options = question.options.asMap().entries.map((entry) => _option(question, entry.key, entry.value)).toList();
    return Scaffold(appBar: AppBar(title: Text(widget.title), actions: [Text('${seconds ~/ 60}:${(seconds % 60).toString().padLeft(2, '0')}'), const SizedBox(width: 16)]), body: Padding(padding: const EdgeInsets.all(20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      LinearProgressIndicator(value: (index + 1) / widget.questions.length), const SizedBox(height: 24),
      Text('Question ${index + 1} of ${widget.questions.length}', style: TextStyle(color: Theme.of(context).colorScheme.primary, fontWeight: FontWeight.bold)), const SizedBox(height: 12),
      Text(question.text, style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold)), const SizedBox(height: 22),
      ...options,
      if (answered) Card(child: Padding(padding: const EdgeInsets.all(14), child: Text(question.explanation))),
      const Spacer(), SizedBox(width: double.infinity, child: FilledButton(onPressed: answered ? _next : null, child: Text(index == widget.questions.length - 1 ? 'Finish quiz' : 'Next question')))
    ])));
  }
...
class ResultPage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Scaffold(appBar: AppBar(title: const Text('Quiz complete')), body: Center(child: Padding(padding: const EdgeInsets.all(24), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(correct >= total / 2 ? Icons.emoji_events : Icons.refresh, size: 78, color: Theme.of(context).colorScheme.primary), const SizedBox(height: 18), Text(title, style: Theme.of(context).textTheme.titleLarge), const SizedBox(height: 8), Text('$correct / $total correct', style: Theme.of(context).textTheme.headlineMedium), const SizedBox(height: 24), FilledButton(onPressed: () => Navigator.pop(context), child: const Text('Back to study'))])));
}
...
class _PlannerState extends State<Planner> {
  ...
  @override
  Widget build(BuildContext context) => Column(children: [AppBar(title: const Text('Study planner'), actions: [IconButton(onPressed: addTask, icon: const Icon(Icons.add))]), Expanded(child: ListView.builder(itemCount: widget.store.tasks.length, itemBuilder: (_, index) => CheckboxListTile(value: false, onChanged: (_) => widget.store.removeTask(index), title: Text(widget.store.tasks[index]), secondary: const Icon(Icons.check_circle_outline))) ]);
}
...
class ProgressPage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Scaffold(appBar: AppBar(title: const Text('Progress')), body: ListView(padding: const EdgeInsets.all(20), children: [Card(child: ListTile(title: const Text('Questions answered'), trailing: Text('${store.solvedToday}'))), Card(child: ListTile(title: const Text('Accuracy'), trailing: Text('${(store.accuracy * 100).round()}%'))), Card(child: ListTile(title: const Text('Bookmarked questions'), trailing: Text('${store.bookmarks.length}')))]);
}
